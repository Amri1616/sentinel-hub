...
export default function LEAUserDetail() {
  ...
  const handleSave = () => {
    ...
    setIsEditing(false);
    toast.success("LEA user profile updated successfully.");
  };

  const toggleStatus = () => {
    const newStatus = userData.status === 'active' ? 'inactive' : 'active';
    setUserData(prev => ({ ...prev, status: newStatus }));
    toast.info(`Agency account ${newStatus === 'active' ? 'activated' : 'deactivated'} successfully.`);
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => navigate('/super-admin/lea-users')}>
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="border-border/40 shadow-sm overflow-hidden">
            <CardHeader className="bg-muted/30 border-b py-4">
              <CardTitle className="text-xs font-bold uppercase tracking-widest text-slate-800">Agency User Information</CardTitle>
            </CardHeader>
            <CardContent className="pt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Officer Full Name</Label>
                ...
                <p className="text-[9px] text-muted-foreground font-medium italic">Managed via HQ records</p>
              </div>

              <div className="space-y-2">
                <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Officer MyKad</Label>
                <div className="flex items-center gap-2 p-3 bg-muted/20 rounded-md border border-slate-100">
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Designation / Rank</Label>
                {isEditing ? (
                  <Input 
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Agency Department</Label>
                {isEditing ? (
                  <Input 
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Official Email</Label>
                <div className="flex items-center gap-2 p-3 bg-muted/20 rounded-md border border-slate-100">
                  <Mail className="h-4 w-4 text-muted-foreground" />
                  <span className="font-semibold text-slate-900">{userData.email}</span>
                </div>
                <p className="text-[9px] text-muted-foreground font-medium italic">Fixed agency domain email</p>
              </div>
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Official Phone</Label>
                <div className="flex items-center gap-2 p-3 bg-muted/20 rounded-md border border-slate-100">
                  <Phone className="h-4 w-4 text-muted-foreground" />
                  <span className="font-semibold text-slate-900">{userData.phone}</span>
                </div>
                <p className="text-[9px] text-muted-foreground font-medium italic">Fixed agency contact</p>
              </div>

              <div className="space-y-2">
                <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Alt Phone (Optional)</Label>
                {isEditing ? (
                  <Input 
                    type="tel"
              ...
              <div className="space-y-2 md:col-span-2">
                <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Agency System Role</Label>
                <div className="flex items-center gap-2 p-3 bg-muted/20 rounded-md border border-slate-100">
              ...
              <div className="space-y-2 md:col-span-2">
                <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">LEA Organization</Label>
                <div className="flex items-center gap-2 p-3 bg-muted/20 rounded-md border border-slate-100">
          ...
          <Card className="border-border/40 shadow-sm bg-blue-500/5 border-blue-500/10">
            <CardContent className="p-4 flex items-start gap-4">
              ...
              <div className="space-y-1">
                <h4 className="text-sm font-bold text-blue-900 uppercase tracking-widest">Agency Policy Constraint</h4>
                <p className="text-xs text-blue-700/80 leading-relaxed">
                  LEA user accounts are formally linked to agency credentials. Core identity markers (Rank, Name, MyKad, Email) are derived from the official LEA registration application.
                </p>
              </div>
            </CardContent>
          </Card>
