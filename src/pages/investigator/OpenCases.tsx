...
const openCases = [
  { id: 'PSIRP-2025-0063', title: 'High-Value Package Theft', org: 'Global Express Logistics', reporter: 'Ali Hassan', officer: 'Raj Kumar', severity: 'Medium', status: 'Under Review', submitted: '2025-06-10' },
  { id: 'PSIRP-2025-0058', title: 'Unauthorized Warehouse Access', org: 'J&T Express', reporter: 'Lim Wei Jie', officer: 'Lee Wei', severity: 'High', status: 'Under Review', submitted: '2025-06-07' },
];

export default function InternalOpenCases() {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Read-only overview of ongoing incident investigations</p>
        </div>
        <Badge variant="outline" className="bg-status-in-review/10 text-status-in-review border-status-in-review/30 px-3 py-1">
          VIEW ONLY
        </Badge>
      </div>
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <Input placeholder="Search open cases..." className="pl-10" value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
              ...
              <tbody className="divide-y divide-border">
                {filtered.map((c) => (
                  <tr key={c.id} className="hover:bg-muted/30 cursor-pointer transition-colors border-b" onClick={() => navigate(`/internal/cases/${c.id}`)}>
                    ...
                    <td className="px-3 py-4 text-center align-middle"><Button size="sm" variant="ghost" onClick={(e) => { e.stopPropagation(); navigate(`/internal/cases/${c.id}`); }}><Eye className="h-4 w-4 mr-2" /> View Case</Button></td>
                  </tr>
                ))}
              </tbody>
            </table>
